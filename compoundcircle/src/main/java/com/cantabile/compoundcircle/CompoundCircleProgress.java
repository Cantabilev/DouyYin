package com.cantabile.compoundcircle;
...
import com.cantabile.compoundcircle.util.Constant;
import com.cantabile.compoundcircle.util.MiscUtil;

/**
 * Created by simple on 2017/12/11.
 */

public class CompoundCircleProgress extends View implements Checkable, View.OnClickListener {

    private static final String TAG = CompoundCircleProgress.class.getSimpleName();
    ...
    private int mBgArcColorOn;
    private int mBgArcColorOff;
    ...
    private int mBgPicColorOn;
    private int mBgPicColorOff;
    private float mBgPicWidth;
    ...
    public CompoundCircleProgress(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CompoundCircleProgress(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mBgArcColorOn = typedArray.getColor(R.styleable.CircleProgressBar_bgArcColorOn, Color.WHITE);
        mBgArcColorOff = typedArray.getColor(R.styleable.CircleProgressBar_bgArcColorOff, Color.WHITE);
        ...
        mBgPicColorOn = typedArray.getColor(R.styleable.CircleProgressBar_bgPicColorOn, Color.WHITE);
        mBgPicColorOff = typedArray.getColor(R.styleable.CircleProgressBar_bgPicColorOff, Color.WHITE);
    ...
    private void initPaint() {
        ...
        mBgArcPaint.setColor(mBgArcColorOff);
        mBgArcPaint.setStyle(Paint.Style.STROKE);
        mBgArcPaint.setStrokeWidth(mBgArcWidth);
        mBgArcPaint.setStrokeCap(Paint.Cap.ROUND);

        mPicPaint = new Paint();
        mPicPaint.setAntiAlias(antiAlias);
        mPicPaint.setColor(mBgPicColorOff);
        mPicPaint.setStyle(Paint.Style.STROKE);
        mPicPaint.setStrokeWidth(mBgArcWidth);
    }
...
//        drawText(canvas);
    ...
    /**
     * 根据实际 checked 情况 绘制 播放按钮 或者 暂停按钮
     * @param canvas
     */
    private void drawPic(Canvas canvas){
        canvas.save();
        if(mChecked){// 画 暂停
            mPicPaint.setColor(mBgPicColorOn);
            canvas.drawLine(mCenterPoint.x - mRadius/5, mCenterPoint.y - mRadius/3, mCenterPoint.x - mRadius/5, mCenterPoint.y + mRadius/3, mPicPaint);
            canvas.drawLine(mCenterPoint.x + mRadius/5, mCenterPoint.y - mRadius/3, mCenterPoint.x + mRadius/5, mCenterPoint.y + mRadius/3, mPicPaint);
        }else {// 画 播放
            mPicPaint.setColor(mBgPicColorOff);
            Path path = new Path();
            path.moveTo(mCenterPoint.x - mRadius/5, mCenterPoint.y - mRadius/3);
            path.lineTo(mCenterPoint.x + mRadius/3, mCenterPoint.y);
            path.lineTo(mCenterPoint.x - mRadius/5, mCenterPoint.y + mRadius/3);
            path.lineTo(mCenterPoint.x - mRadius/5, mCenterPoint.y - mRadius/3);
            path.close();
            canvas.drawPath(path, mPicPaint);
        }
        canvas.restore();
    }
    ...
    private void drawArc(Canvas canvas) {
        ...
        if(mChecked)
            mBgArcPaint.setColor(mBgArcColorOn);
        else
            mBgArcPaint.setColor(mBgArcColorOff);
    ...
    public interface OnCheckedChangeListener {
        void onCheckedChanged(CompoundCircleProgress view, boolean checked);
    }
}
